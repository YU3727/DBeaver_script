...
--사원의 번호가 100번인 사람의 부서번호와 first_name이 알고싶다
--참조해야 하는 TABLE이 2개 이상일때 : JOIN 기능을 사용
--원래 규칙은 FROM 뒤에 하나의 TABLE만 온다.
...
--같은 데이터가 2개 이상의 테이블에 있는 경우(DEPARTMENT_ID), 참조할 테이블의 별칭을 붙이지 않으면 에러가 뜨기 때문에 별칭을 꼭 붙여주자.

--2교시
--부서번호가 30번인 부서에 근무하는 사원의 정보(이름, 월급, 부서명, 부서아이디) 출력
...
FROM DEPARTMENTS D
	INNER JOIN
	EMPLOYEES E
	ON (E.DEPARTMENT_ID = D.DEPARTMENT_ID AND E.DEPARTMENT_ID = 30); --ON에서는 AND 사용가능

--부서번호가 30번인 부서에 근무하는 사원의 정보(이름, 월급, 부서명, 부서아이디, 도시명, 나라명, 대륙명) 출력
SELECT E.FIRST_NAME , E.SALARY , D.DEPARTMENT_ID , D.DEPARTMENT_NAME , L.CITY , C.COUNTRY_NAME , R.REGION_NAME 
FROM DEPARTMENTS D
	INNER JOIN
	EMPLOYEES E
	ON (E.DEPARTMENT_ID = D.DEPARTMENT_ID) -- 두 TABLE을 JOIN한 결과물을 또 다른 가상의 TABLE이라 생각한다.
	INNER JOIN 
	LOCATIONS L 
	ON (D.LOCATION_ID = L.LOCATION_ID) -- 가상의 TABLE과 LOCATIONS TABLE을 다시 JOIN함.
	INNER JOIN 
	COUNTRIES C 
...
-- USING
-- JOIN 조건중에서 COLUMN명이 같을 경우 사용한다.
SELECT E.FIRST_NAME , D.DEPARTMENT_NAME , DEPARTMENT_ID 
FROM EMPLOYEES E
	INNER JOIN
	DEPARTMENTS D 
	USING(DEPARTMENT_ID) --USING에서 사용한 컬럼명은 사용시 테이블명의 식별자를 사용하지 않는다.(TABLE을 구분하지 않게됨)
WHERE E.EMPLOYEE_ID = 100;
...
FROM EMPLOYEES E
	INNER JOIN
	DEPARTMENTS D 
	USING(DEPARTMENT_ID --AND E.EMPLOYEE_ID = 100) USING에서는 AND를 사용할 수 없음
WHERE E.EMPLOYEE_ID = 100;
...
SELECT E.FIRST_NAME , E.SALARY , LOCATION_ID , L.CITY 
FROM EMPLOYEES E
	INNER JOIN
	DEPARTMENTS D 
	ON (E.DEPARTMENT_ID = D.DEPARTMENT_ID)
...
--이렇게 SELF JOIN을 써야하는 경우도 있다.


--부서명, 부서관리자의 이름, 월급, 입사일 출력
--조건이 FALSE일때도 한 쪽 DATA를 보여주고 싶을 때 : OUTER JOIN (단, OUTER는 생략 가능. LEFT JOIN, RIGHT JOIN으로 표시가능)
--어느쪽에 보여주고싶은 DATA가 있냐에 따라서 LEFT, RIGHT 추가
--양쪽에 모두 DATA가 있어서 둘 다 보여줘야 할때는 FULL
SELECT * FROM DEPARTMENTS D2 ;

SELECT D.DEPARTMENT_NAME , E.EMPLOYEE_ID , E.SALARY , E.HIRE_DATE  
FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E	--OUTER는 조건식이 FALSE여도 데이터가 있으면 보여주는 것인데, 데이터가 있는 방향이 필요하다.
	ON(D.DEPARTMENT_ID = E.MANAGER_ID);			--여기서, DATA는 DEPARTMENTS TABLE에 있으므로 왼쪽이라고 써준다.
...
--------------------------------------
--최소급여가 8200, 최대급여가 16000 사이에 속하는 JOB_TITLE과 사원의 ID, 급여
SELECT * FROM JOBS;
SELECT * FROM EMPLOYEES e ;

--EMPLOYEES TABLE과 JOBS TABLE의 조건이 연결이 안될때가 있다.
--조건이 연결이 안되기 때문에 같다틀리다 같은 비교가 불가능하다. 즉, 같은/틀린걸 찾는게 아니라 범위에 있는걸 찾아야한다.
--Non-Equal Join - 공통적인 컬럼 없이 범위를 사용해서
...
FROM EMPLOYEES E
	 ...
	 DEPARTMENTS D
;
--WHERE EMPLOYEE_ID = 100; --NATURAL JOIN은 WHERE이 작동하지 않음.
...
--Table 생성
CREATE TABLE TEST(
	NAME VARCHAR2(100), --VARCHAR2는 가변길이문자열. 공간이 남으면 줄임.
	NUM NUMBER, --NUMBER(전체자리수, 소수점자리수) 아무것도 안쓰면 최대 38자리
	KOR NUMBER(3),
	AVG1 NUMBER(5, 2) --정수부3자리, 소숫점2자리까지 허용
);
...
INSERT INTO TEST (NAME, NUM, KOR, AVG1)
VALUES ('Richard', 1, 50, 30);
